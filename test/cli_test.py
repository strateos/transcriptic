...
def test_login_nonexistent_key_path(cli_test_runner):
    result = cli_test_runner.invoke(
        cli, ['login', '--rsa-key', '/temp/path/invalid_key.pem'],
...
def test_login_random_file_for_key(cli_test_runner, tmpdir_factory):
    ...
    result = cli_test_runner.invoke(
        cli, ['login', '--rsa-key', str(path)],
        input="\n".join(["email@foo.com", "barpw"])
    )
    assert("Error loading RSA key: Could not parse the specified RSA Key, "
           "ensure it is a PRIVATE key in PEM format" in
           result.output)
...
def test_login_public_key(cli_test_runner, temp_ssh_key):
    ...
    result = cli_test_runner.invoke(
        cli, ['login', '--rsa-key', public_key_path],
        input="\n".join(["email@foo.com", "barpw"])
    )
    assert("Error connecting to host: This is not a private key" in
           result.output)
